...
if(! defined('MAXWM_WIDTH') )				define ('MAXWM_WIDTH', 180);							// Default thumbnail width. Allows overrid in timthumb-config.php
if(! defined('MAXWM_HEIGHT') )			    define ('MAXWM_HEIGHT', 180);							// Default thumbnail height. Allows overrid in timthumb-config.php
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
        ...
        if($new_width<=MAXWM_WIDTH && $new_height<=MAXWM_HEIGHT){
            // reflected
            
            $bn = basename($localImage);
            $try = preg_replace("|".$bn."|", "/originals/".$bn, $localImage);
            if(file_exists($try)){
                $localImage = $try;
            }
